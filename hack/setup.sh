...
readonly GIT_SERVE_VERSION=0.5.0
...
start_git_serve() {
  kapp deploy --yes -a git-serve -f https://github.com/cirocosta/git-serve/releases/download/v0.0.5/git-serve.yaml
  ytt -f hack/git-server | kubectl apply -f -
}
...
add_ssh_token() {
  if [[ -z ${GIT_WRITER_SSH_TOKEN+nullword} ]]; then
    if [[ -f hack/git-server/git-ssh/client ]]; then
      GIT_WRITER_SSH_TOKEN="$(cat hack/git-server/git-ssh/client)"
    else
      return 1
    fi
  fi
  ...
  ssh-add -t 1000 - <<<"$GIT_WRITER_SSH_TOKEN" 2>/dev/null || {
    mkdir -p ~/.ssh
    cp hack/git-server/git-ssh/client ~/.ssh/id_rsa
    chmod 600 ~/.ssh/id_rsa
  }
  log "here"

  GIT_WRITER_SERVER_PUBLIC_TOKEN=$(cat hack/git-server/git-ssh/server.pub)
...
setup_source_to_gitops() {
  ...
  kapp deploy --yes -a "example-$test_name" \
    -f <(ytt --ignore-unknown-comments \
      -f "$DIR/../examples/$test_name" \
      --data-value git_repository="http://git-serve:80/gitops-test.git" \
      --data-value git_branch="main" \
      --data-value git_user_name="gitops-user" \
      --data-value git_user_email="gitops-user@example.com" \
      --data-value git_commit_message="Update config")
}
...
test_source_to_gitops() {
  ...
  port=$(available_port)
  kubectl port-forward service/git-serve $port:80 &

  EXPECTED_GIT_MESSAGE="Update config"
    ...
    pushd "gitops-test"
      for i in {20..1}; do
        ...
        if [[ ! "$(git branch --show-current)" = "main" ]]; then
          git checkout "main" >/dev/null 2>/dev/null || sleep "$i" && continue
        fi
